...
namespace WpfSimpleViewManager.Dialog
{
    internal class DialogService : IDialogService, IDialogRegister
    {
        ...
        public void ShowDialog(string viewName, Parameters? parameters = null, Action<IDialogResult>? callback = null, StartPosition? startPosition = null)
        {
            ...
            dialogBase.ShowDialog();
        }

        public void Show(string viewName, Parameters? parameters,  StartPosition? startPosition = null)
        {
            //TODO : 로깅
            IsViewNameValid(viewName);
        ...
        private T? FindChild<T>(DependencyObject parent, string name) where T : DependencyObject
        {
            ...
            {
                parent = contentControl.Content as DependencyObject ?? throw new ArgumentException("Dialog는 UserControl 이여야 합니다.");
            }
